...
在完成系统架构设计、核心子系统实现与关键机制优化后，需要从实验角度验证 StarryOS 的实际表现。本章围绕功能完整性、应用兼容性与性能三个维度展开评测：首先通过操作系统大赛标准测例检验系统调用覆盖范围与语义正确性；其次在实际应用场景中验证开发工具链、图形环境及典型服务的运行情况；最后针对系统调用开销、I/O吞吐等关键指标进行性能基准测试，并结合第4章的优化机制分析其效果。
